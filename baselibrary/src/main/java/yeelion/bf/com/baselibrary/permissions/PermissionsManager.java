...
public class PermissionsManager {
    ...
    public static final int PERMISSION_RESULT_CALL_BACK_CODE = 52011;
    ...
    private synchronized void initializePermissionsMap() {
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                Logger.e(TAG, "Could not access field");
            }
    ...
    private synchronized String[] getManifestPermissions(@NonNull final Activity activity) {
        ...
        try {
            Logger.i(TAG, activity.getPackageName());
            packageInfo = activity.getPackageManager().getPackageInfo(activity.getPackageName(), PackageManager.GET_PERMISSIONS);
        } catch (PackageManager.NameNotFoundException e) {
            Logger.e(TAG, "A problem occurred when retrieving permissions");
        }
        if (packageInfo != null) {
            String[] permissions = packageInfo.requestedPermissions;
            if (permissions != null) {
                for (String perm : permissions) {
                    Logger.i(TAG, "Manifest contained permission: " + perm);
                    list.add(perm);
                }
            }
        }
    ...
    public synchronized void requestPermissionsIfNecessaryForResult(@Nullable Activity activity,
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            if (permList.isEmpty()) {
                ...
                // if need permission is empty, but the permission not request sucess, so rerequest it.
                String[] notPerimissions = new String[permissions.length];
                for (int i = 0; i < permissions.length; i++) {
                    boolean hasPermission = hasPermission(activity, permissions[i]);
                    if (hasPermission) {
                        removePendingAction(action);
                    } else {
                        notPerimissions[i] = permissions[i];
                    }
                }
                if (notPerimissions.length > 0) {
                    ActivityCompat.requestPermissions(activity, notPerimissions, PERMISSION_RESULT_CALL_BACK_CODE);
                }
            } else {
                String[] permsToRequest = permList.toArray(new String[permList.size()]);
                mPendingRequests.addAll(permList);
                ActivityCompat.requestPermissions(activity, permsToRequest, PERMISSION_RESULT_CALL_BACK_CODE);
            }
        }
    }
    ...
    public synchronized void requestPermissionsIfNecessaryForResult(@NonNull Fragment fragment,
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            if (permList.isEmpty()) {
                ...
                String[] permsToRequest = permList.toArray(new String[permList.size()]);
                mPendingRequests.addAll(permList);
                fragment.requestPermissions(permsToRequest, PERMISSION_RESULT_CALL_BACK_CODE);
            }
        }
    }

    public boolean checkoutPerimissionRationable(Activity activity, String perimission) {
        return ActivityCompat.shouldShowRequestPermissionRationale(activity, perimission);
    }
